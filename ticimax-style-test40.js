(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const c of document.querySelectorAll('link[rel="modulepreload"]'))o(c);new MutationObserver(c=>{for(const d of c)if(d.type==="childList")for(const p of d.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&o(p)}).observe(document,{childList:!0,subtree:!0});function u(c){const d={};return c.integrity&&(d.integrity=c.integrity),c.referrerPolicy&&(d.referrerPolicy=c.referrerPolicy),c.crossOrigin==="use-credentials"?d.credentials="include":c.crossOrigin==="anonymous"?d.credentials="omit":d.credentials="same-origin",d}function o(c){if(c.ep)return;c.ep=!0;const d=u(c);fetch(c.href,d)}})();console.log("🔵 Checkout Tracking Script Başladı");console.log("🔵 Sayfa:",window.location.pathname);if(!window.location.pathname.includes("/checkout"))console.log("🔵 Checkout sayfası değil, script sonlandırılıyor.");else{const a="uvid",r=localStorage.getItem(a);if(console.log("🔵 visitorId (uvid):",r),!r)console.log("🔵 visitorId bulunamadı, işlem iptal edildi.");else{let u=function(){o++;const p=window.checkout_order;console.log(`🟡 Attempt ${o}/${c}`,{checkout_order:p});const g=p?.id??p?.orderId??null;if(g){console.log("🟢 OrderId BULUNDU:",g),console.log("🟢 API isteği gönderiliyor...");const w=new XMLHttpRequest;return w.open("POST","https://api.upsell.co/Tracking/MatchOrder",!0),w.setRequestHeader("Content-Type","application/json;charset=UTF-8"),w.onreadystatechange=function(){w.readyState===4&&(w.status>=200&&w.status<300?(console.log("✅ BAŞARILI! visitorId siliniyor."),localStorage.removeItem(a)):console.error("❌ MatchOrder HATASI:",w.responseText))},w.send(JSON.stringify({visitorId:r,orderId:String(g)})),!0}return!1},o=0;const c=50;if(console.log("🔵 İlk kontrol başlatılıyor..."),u())console.log("🟢 İlk kontrolde order bulundu!");else{console.log("🟡 Order bulunamadı, polling başlatılıyor...");const d=setInterval(()=>{u()?(console.log("🟢 Polling sırasında order bulundu, durduruluyor."),clearInterval(d)):o>=c&&(console.log("⏱️ Maksimum deneme sayısına ulaşıldı, polling durduruluyor."),clearInterval(d))},100)}}}var Ds=typeof globalThis<"u"?globalThis:typeof window<"u"?window:typeof global<"u"?global:typeof self<"u"?self:{};function Bx(a){return a&&a.__esModule&&Object.prototype.hasOwnProperty.call(a,"default")?a.default:a}var Ec={exports:{}},Kl={};/**
 * @license React

...
`,O2=({mainColor:i,stockReminderUsageChannel:r,forceOpen:o})=>{const s=Y.useRef(null),[c,d]=Y.useState(o||!1),[p,g]=Y.useState(!1),[w,x]=Y.useState(""),[v,D]=Y.useState(!1),[E,Q]=Y.useState(),[U,V]=Y.useState(),[M,ee]=Y.useState(!1),q=Y.useRef(null),X=Y.useRef(null),ie=Y.useRef(null),B=()=>{document.querySelectorAll(".nostok").forEach(y=>{if(!y.querySelector(".stock_bell_icon")){const C=document.createElement("i");C.className="fal fa-bell stock_bell_icon",C.style.fontSize="14px",C.style.color="#000",C.style.fontWeight="500",C.style.marginLeft="5px",C.style.transition="0.3s",y.appendChild(C)}})},te=()=>{ie.current&&clearInterval(ie.current),ie.current=setInterval(()=>{document.querySelectorAll(".stock_bell_icon").forEach(y=>{const C=y;C.style.transform="scale(1.5)",C.style.color="red",C.style.transition="0.3s",setTimeout(()=>{C.style.transform="scale(1)",C.style.color="#000",C.style.transition="0.3s"},1e3)})},3e3)},K=()=>{const ne=document.getElementById("dynamicNostokBellStyles");ne&&ne.remove();const y=document.createElement("style");y.id="dynamicNostokBellStyles",y.innerHTML=`
      span.nostok {
        display: flex !important;

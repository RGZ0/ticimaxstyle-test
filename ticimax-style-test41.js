(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const c of document.querySelectorAll('link[rel="modulepreload"]'))o(c);new MutationObserver(c=>{for(const d of c)if(d.type==="childList")for(const p of d.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&o(p)}).observe(document,{childList:!0,subtree:!0});function u(c){const d={};return c.integrity&&(d.integrity=c.integrity),c.referrerPolicy&&(d.referrerPolicy=c.referrerPolicy),c.crossOrigin==="use-credentials"?d.credentials="include":c.crossOrigin==="anonymous"?d.credentials="omit":d.credentials="same-origin",d}function o(c){if(c.ep)return;c.ep=!0;const d=u(c);fetch(c.href,d)}})();(function(){if(console.log("🔵 Checkout Tracking Script Başladı"),console.log("🔵 Sayfa:",window.location.pathname),!window.location.pathname.includes("/checkout")){console.log("🔵 Checkout sayfası değil, script sonlandırılıyor.");return}let a=0;const r=setInterval(function(){console.log("🟡 Kontrol ediliyor... Geçen süre:",a+"ms","checkout_order:",window.checkout_order),(window.checkout_order||a>=5e3)&&(clearInterval(r),console.log("🟢 Interval durdu. checkout_order bulundu mu?",!!window.checkout_order),u()),a+=100},100);function u(){const o=window.checkout_order,c="uvid",d=localStorage.getItem(c);if(console.log("📊 processOrder çağrıldı"),console.log("📊 visitorId (uvid):",d),console.log("📊 checkout_order:",o),!o||!d){console.error("❌ Conversion-tracking: checkout_order veya visitorId bulunamadı.");return}console.log("✅ Step 1 - Veri başarıyla yakalandı.");const p=o.id;if(console.log("📦 orderId:",p),p){console.log("✅ Step 2 - OrderId mevcut");const g=new XMLHttpRequest;g.open("POST","https://api.upsell.co/Tracking/MatchOrder",!0),g.setRequestHeader("Content-Type","application/json;charset=UTF-8"),g.onreadystatechange=function(){g.readyState===4&&(g.status>=200&&g.status<300?(console.log("✅ Conversion-tracking: Başarılı. Ziyaretçi anahtarı siliniyor."),localStorage.removeItem(c)):console.error("❌ Conversion-tracking MatchOrder error:",g.responseText))},console.log("📤 Step 3 - İstek gönderiliyor."),g.send(JSON.stringify({visitorId:d,orderId:String(p)})),console.log("✅ Step 4 - İstek gönderildi.")}}})();var Ds=typeof globalThis<"u"?globalThis:typeof window<"u"?window:typeof global<"u"?global:typeof self<"u"?self:{};function Bx(a){return a&&a.__esModule&&Object.prototype.hasOwnProperty.call(a,"default")?a.default:a}var Ec={exports:{}},Kl={};/**
 * @license React
